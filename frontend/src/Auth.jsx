...
export function AdminDashboard({currentUser,onClose,onLogout}) {
  ...
  const load=()=>Promise.all([api("/api/admin/users"),api("/api/admin/settings")]).then(([a,b])=>{setUsers(a.users);setRegistration(b.registration_enabled)}).catch(e=>setError(e.message));
  useEffect(()=>{load()},[]);
  ...
  return <div className="admin-page"><header className="admin-head"><div><div className="eyebrow">ADMINISTRATION</div><h1>User performance</h1><p>Quản lý tài khoản và hiệu suất labeling lưu trên server.</p></div><div><button className="ghost" onClick={onClose}>← Workspace</button><button className="ghost" onClick={onLogout}>Logout</button></div></header><section className="admin-summary"><div><b>{users.length}</b><span>Users</span></div><div><b>{users.reduce((n,u)=>n+Number(u.images_saved||0),0).toLocaleString()}</b><span>Images saved</span></div><div><b>{users.reduce((n,u)=>n+Number(u.boxes_saved||0),0).toLocaleString()}</b><span>Boxes saved</span></div><label><input type="checkbox" checked={registration} onChange={toggleRegistration}/> Cho phép đăng ký mới</label></section>{error&&<div className="auth-error">{error}</div>}<section className="admin-table-wrap"><table className="admin-table"><thead><tr><th>User</th><th>Role</th><th>Images</th><th>Boxes</th><th>Prelabel</th><th>Active time</th><th>Last active</th><th>Account</th><th>Actions</th></tr></thead><tbody>{users.map(user=><tr key={user.id}><td><b>{user.display_name}</b><small>@{user.username}</small></td><td><select value={user.role} disabled={user.id===currentUser.id} onChange={e=>update(user,{role:e.target.value})}><option value="annotator">annotator</option><option value="admin">admin</option></select></td><td>{Number(user.images_saved||0).toLocaleString()}</td><td>{Number(user.boxes_saved||0).toLocaleString()}</td><td>{Number(user.prelabel_runs||0).toLocaleString()} / {Number(user.prelabel_images||0).toLocaleString()} ảnh</td><td>{duration(Number(user.active_seconds||0))}</td><td>{user.last_active?new Date(user.last_active*1000).toLocaleString():"—"}</td><td><span className={user.active?"user-active":"user-disabled"}>{user.active?"Active":"Disabled"}</span>{user.must_change_password&&<small>Must change password</small>}</td><td><button onClick={()=>reset(user)}>Reset password</button><button disabled={user.id===currentUser.id} onClick={()=>update(user,{active:!user.active})}>{user.active?"Disable":"Enable"}</button></td></tr>)}</tbody></table></section></div>;
}
